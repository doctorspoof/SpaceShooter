...
public class EnemyGroup : MonoBehaviour
{
	...
	public void SortShipsWithRegardsToFormation(out List<EnemyScript> inFormation, out List<EnemyScript> outOfFormation)
	{
		...
		foreach (List<EnemyScript> shipTier in m_children)
		{
			List<EnemyScript> inTier = shipTier.FindAll(delegate(EnemyScript enemy)
			                                            {
				if (enemy.GetDistanceFromFormation() <= 0.8f)
				{
					return true;
				}
				return false;
			...
			inFormation.AddRange(inTier);
			
			List<EnemyScript> outTier = shipTier.FindAll(delegate(EnemyScript enemy)
			                                             {
				if (enemy.GetDistanceFromFormation() > 0.8f)
				{
					return true;
			...
			outOfFormation.AddRange(outTier);
		}
	}
	...
	public bool InFormation(out EnemyScript shipFurthestOutOfFormation)
	{
		bool inFormation = true;
		shipFurthestOutOfFormation = null;
		foreach (List<EnemyScript> shipTier in m_children)
		{
			foreach (EnemyScript enemy in shipTier)
			{
				...
				{
					inFormation = false;
					if (shipFurthestOutOfFormation == null || shipFurthestOutOfFormation.GetDistanceFromFormation() < enemy.GetDistanceFromFormation())
					{
