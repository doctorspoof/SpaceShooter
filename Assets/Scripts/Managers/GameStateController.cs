...
public class GameStateController : MonoBehaviour 
{
	...
	public List<DeadPlayer> m_deadPlayers;

	[SerializeField]
	GameObject m_SpawnManager;
	[SerializeField]
	GameObject m_GUIManager;
	[SerializeField]
	GameObject[] m_AsteroidManagers;

	public GameState m_currentGameState = GameState.MainMenu;
	[SerializeField]
	...
	[SerializeField]
	GameObject m_ingameCapitalShip;
	...
	public bool m_shouldCheckForFinished = false;
	// Use this for initialization
	void Start () 
	{
		volumeHolder = PlayerPrefs.GetFloat("MusicVolume", 1.0f);
	...
	public void StartGameFromMenu(bool isSpecMode)
	{
		...
		//Initialising spawns
		//m_SpawnManager.GetComponent<EnemySpawnManagerScript>().InitSpawnPoints();
		//m_SpawnManager.GetComponent<EnemySpawnManagerScript>().BeginSpawning();
	...
	[RPC]
	void SpawnAShip(NetworkPlayer player)
	{
		if(Network.isClient)
		{
		...
		pos.z = 10.75f;
		//pos.z = 10.0f;
		GameObject ship = (GameObject)Network.Instantiate(playerShip, pos, m_ingameCapitalShip.transform.rotation, 0);
	...
	void SpawnCapitalShip()
	{
		...
		GameObject capital = (GameObject)Network.Instantiate(capitalShip, spawnPoint.transform.position, spawnPoint.transform.rotation, 0);
	...
	bool shouldSpawnRoids = true;
	[RPC]
	void ChangeToInGame()
	{
		if(Network.isServer && shouldSpawnRoids)
		{
	...
	[RPC]
	void PropagatePlayerRemoval(NetworkPlayer player)
	{
		for(int i = 0; i < m_connectedPlayers.Count; i++)
		{
			if(m_connectedPlayers[i].m_netPlayer == player)
	...
	[RPC]
	void ClientAsksHostToSpreadName(string name, NetworkMessageInfo info)
	{
	...
	[RPC]
	void ServerSpreadsName(string name, NetworkPlayer player)
	{
	...
	[RPC]
	void PlayerSendsNameToOthers(string name, NetworkMessageInfo info)
	{
	...
	[RPC]
	void ServerPassOtherClientName(string name, NetworkPlayer player)
	{
		Debug.Log ("Recieved name storage request: Player " + player + " with name: '" + name + "'");
		m_connectedPlayers.Add(new Player(player, name));
	}
	[RPC]
	void PlayerPassbackName(string name, NetworkMessageInfo info)
	{
	...
	[RPC]
	void TellAllDeadPlayersRespawn()
	{
		if(localPlayer == null)
		{
	...
	void OnConnectedToServer()
	{
		...
		m_connectedPlayers.Add (new Player(Network.player, ownName));
		Debug.Log ("Sending our name: '" + ownName + "' to other clients.");
		networkView.RPC ("PlayerSendsNameToOthers", RPCMode.Server, ownName);
	...
	[RPC]
	void NotifyClientsPlayersHaveDied()
	{
	...
	[RPC]
	void CapitalShipArrivesAtVictoryPoint()
	{
		//Stop all enemies
		...
		m_ingameCapitalShip.GetComponent<CapitalShipScript>().shouldStart = false;
	...
	[RPC]
	void PropagateCapitalShipUnderFire()
	{
	...
	[RPC]
	void CapitalShipHasBeenDestroyed()
	{
		//TODO:
		//Add loss state
		//On loss:
	...
	[RPC]
	void PlayerConfirmsGameOver(NetworkMessageInfo info)
	{
		for(int i = 0; i < m_lossConfirmList.Count; i++)
		{
