...
public class AttackStrafeRight : IAttack
{

    public override void Attack(EnemyScript ship, GameObject target)
    {
		float timeTakenToTravel = Vector3.Distance(target.transform.position, ship.m_shipTransform.position) / ship.GetComponent<EnemyWeaponScript>().GetBulletSpeed();
        ...
        Vector3 direction = Vector3.Normalize(target.rigidbody.position - ship.m_shipTransform.position);
        ...
        {
            ship.rigidbody.AddForce(ship.m_shipTransform.right * ship.GetCurrentMomentum() * Time.deltaTime);
